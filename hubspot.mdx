...
Stigg's integration with HubSpot requires the following HubSpot permissions, which you'll be asked to grant when [adding the integration in Stigg](#adding-the-hubspot-integration-in-stigg):

1. Manage and view your CRM data  
   i. View details about property settings for contacts.  
   ii. Create, delete, or make changes to property settings for contacts.  
   iii. View properties and other details about line items.  
   iv. Create, delete, or make changes to line items.  
   v. View properties and other details about contacts.  
   vi. Create, delete, or make changes to contacts.  
   vii. View properties and other details about companies.  
   viii. Create, delete, or make changes to companies.  
   ix. View properties and other details about deals.  
   x. Create, delete, or make changes to deals.
2. Access to e-commerce  
   i. This includes access to e-commerce features - this permission is required for propagation of your pricing to the HubSpot product catalog.
3. Create or modify all your CRM data through bulk imports  
   i. This app can import records into your CRM - including creation of new records or modification of any of your existing records for all CRM data types (contacts, companies, deals, tickets, etc.). This doesn't include archiving or deleting any data.
  
## Adding the HubSpot integration in Stigg

In the [Stigg Console](), under the **Settings > Integrations > Apps** section, select the HubSpot connector.
...
In the [Stigg Console](), entities that are synced from Stigg to HubSpot have a clickable "CRM ID" property with a HubSpot logo next to it and indication of their sync status (successful, failed, [will not be synced](#entities-that-are-not-synced-to-hubspot)).
...
In the [Stigg Console](), clicking on the link of the "CRM ID" property will open the relevant entity in HubSpot.

<Note> 
Due to the fact that HubSpot doesn't support direct deep-links to entities in the HubSpot product catalog, attempting to access synced **plans** and **add-ons** from the Stigg console will only open the HubSpot product catalog, and not the specific entity inside the product catalog.
</Note>
...
To remove the integration, in the [Stigg Console](), under the **Settings > Integrations > Apps** section, select the HubSpot connector.
...
<Note>
If any of the above mentioned limitations affect your use-case, [let us know](mailto:support@stigg.io) and we'd be happy to work together to lift them.
</Note>
