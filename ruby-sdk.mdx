...
This Ruby gem provides a wrapper to [Stigg's GraphQL API](/graphql-api) based on the operations that are in use by the [Stigg's Node.js SDK](/nodejs-sdk). It leverages the [graphlient](https://github.com/ashkan18/graphlient) library under the hood and utilizes the `Graphlient::Client` class to execute the API requests.
...
resp = client.request(Stigg::Mutation::EstimateSubscription, {
    "input": {
        ...
        "planId": "plan-revvenu-essentials",
        "priceUnitAmount": 4,            # optional, required for plans with per-unit pricing
        "addons": [                                  # optional
            {
